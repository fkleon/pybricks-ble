...
class TestLEAdvertising:
    ...
    def test_advertisement_path(self, name: str, index: int, path: str) -> None:
        adv = LEAdvertisement(
...
class TestLEAdvertisingManager:
    ...
    @pytest.fixture
    def adv_manager(self, adapter: ProxyObject) -> LEAdvertisingManager:
        return LEAdvertisingManager(adapter)

    @pytest_asyncio.fixture
    async def adv(
        self, adv_manager: LEAdvertisingManager
    ) -> AsyncGenerator[LEAdvertisement, None]:
    ...
    async def test_create(self, adapter: ProxyObject) -> None:
    ...
    async def test_register_advertisement(
        self, adv_manager: LEAdvertisingManager, adv: LEAdvertisement
    ) -> None:
        await adv_manager.register_advertisement(adv)

    async def test_unregister_advertisement(
        self, adv_manager: LEAdvertisingManager, adv: LEAdvertisement
    ) -> None:
        await adv_manager.register_advertisement(adv)
        await adv_manager.unregister_advertisement(adv)

    async def test_unregister_advertisement_by_path(
        self, adv_manager: LEAdvertisingManager, adv: LEAdvertisement
    ) -> None:
        await adv_manager.register_advertisement(adv)
        await adv_manager.unregister_advertisement(adv.path)

    async def test_unregister_advertisement_does_not_exist(
        self, adv_manager: LEAdvertisingManager
    ) -> None:
