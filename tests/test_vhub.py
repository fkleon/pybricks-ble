...
class TestVirtualBLE:
    async def test_create_vble(self, adapter: ProxyObject) -> None:
    ...
    async def test_observe_none(self) -> None:
        ble = await get_virtual_ble(broadcast_channel=1, observe_channels=[2])
        data = ble.observe(2)
        assert data is None
        assert not ble._broadcaster.is_broadcasting()

    async def test_observe_single(self, mocker: MockerFixture) -> None:
        ble = await get_virtual_ble(broadcast_channel=1, observe_channels=[2])
    ...
    async def test_observe_multiple(self, mocker: MockerFixture) -> None:
        ble = await get_virtual_ble(broadcast_channel=1, observe_channels=[2])
        observe_mock = mocker.patch.object(ble._observer, "observe")
    ...
    async def test_broadcast_single(self) -> None:
        ble = await get_virtual_ble(broadcast_channel=1, observe_channels=[2])
        await ble.broadcast(42)
        assert ble._broadcaster.is_broadcasting()
        assert ble._adv.message == 42

    async def test_broadcast_multiple(self) -> None:
        ble = await get_virtual_ble(broadcast_channel=1, observe_channels=[2])
        await ble.broadcast(42, 24)
        assert ble._broadcaster.is_broadcasting()
        assert ble._adv.message == (42, 24)

    async def test_broadcast_none(self) -> None:
        ble = await get_virtual_ble(broadcast_channel=1, observe_channels=[2])
        await ble.broadcast(None)
        assert not ble._broadcaster.is_broadcasting()

    async def test_broadcast_start_stop(self) -> None:
        ble = await get_virtual_ble(broadcast_channel=1, observe_channels=[2])
        await ble.broadcast(42)
        assert ble._broadcaster.is_broadcasting()
        await ble.broadcast(None)
